...
import com.vyatsu.task14.Security.services.ProductsService;
...
//    }
    ...
    public String showOneProduct(Model model, @PathVariable(value = "id") long id)
    {
        Product product = productsService.getbyId(id);
        model.addAttribute("product",product);
        System.out.println("Оценили товар с id " + id);
        ...
        Long max = County.entrySet()
                ...
                .max((entry1,
                      entry2) -> entry1.getValue() >
                        entry2.getValue() ? 1 : -1)
                .get().getKey();
        System.out.println("Наиболее оцененый id товара " + max +" кол-во лайков "+County.get(max));
        return "product-page";
    }
